  ...
  if (pipeline_id.empty()) {
    ...
    if (current_key == kPrepareTemplateStart &&
      ...
      pushMetric(kFCP, kPrepareTemplateStart, kPaintEnd,
                 init_timing_info_.GetTimestamp(kPrepareTemplateStart).value(),
    ...
    if (current_key == kOpenTime &&
      ...
      pushMetric(kTotalFCP, kOpenTime, kPaintEnd,
                 init_timing_info_.GetTimestamp(kOpenTime).value(),
                 load_bundle_paint_end);
    }
  } else {
    // check ready
    ...
    pushMetric(kLynxFCP, kLoadBundleStart, kPaintEnd, load_bundle_start.value(),
               paint_end.value());
    ...
    if (prepare_template_start.has_value()) {
      pushMetric(kFCP, kPrepareTemplateStart, kPaintEnd,
                 prepare_template_start.value(), paint_end.value());
    }
    auto open_time = init_timing_info_.GetTimestamp(kOpenTime);
    if (open_time.has_value()) {
      pushMetric(kTotalFCP, kOpenTime, kPaintEnd, open_time.value(),
                 paint_end.value());
    }
  }
...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricTtiEntry(
  ...
  if (pipeline_id.empty()) {
    ...
    pushMetric(kLynxTTI, kLoadBundleStart, kPaintEnd, kLoadBackgroundEnd,
               load_bundle_start.value(), paint_end.value(),
    ...
    if (open_time.has_value()) {
      pushMetric(kTotalTTI, kOpenTime, kPaintEnd, kLoadBackgroundEnd,
                 open_time.value(), paint_end.value(), load_background_end_val);
    }
  }
  ...
  if (pipeline_id.empty()) {
    ...
    if (current_key == kOpenTime &&
      ...
      pushMetric(kTotalActualFMP, kOpenTime, kPaintEnd,
                 init_timing_info_.GetTimestamp(kOpenTime).value(), paint_end);
    }
  } else {
    // check ready
    static const std::initializer_list<std::string> check_keys = {kPaintEnd};
    ...
    pushMetric(kLynxActualFMP, kLoadBundleStart, kPaintEnd,
               load_bundle_start.value(), paint_end.value());
    ...
    auto open_time = init_timing_info_.GetTimestamp(kOpenTime);
    if (open_time.has_value()) {
      pushMetric(kTotalActualFMP, kOpenTime, kPaintEnd, open_time.value(),
                 paint_end.value());
    }
  }
